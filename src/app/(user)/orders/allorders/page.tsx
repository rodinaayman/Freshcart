  ...
  useEffect(() => {
    ...
    const fetchOrders = async () => {
      ...
      if (session && userId && accessToken) {
        try {
          const res = await getUserOrders(userId, accessToken);
      ...
      <div className="space-y-4">
        {orders.length === 0 ? (
          <div className="bg-white rounded-2xl border border-gray-100 p-12 text-center shadow-sm">
            ...
            <p className="text-gray-500 mb-6">Looks like you havent placed any orders.</p>
                        ...
                        {order.cartItems && order.cartItems.map((item, idx) => (
                          <div key={idx} className="flex items-center gap-4 p-4 bg-white rounded-xl border border-gray-100">
                            <div className="w-16 h-16 rounded-xl bg-gray-50 p-2 shrink-0">
                              <img alt={item.product?.title || "Product"} className="w-full h-full object-contain" src={item.product?.imageCover} />
                            </div>
                            <div className="flex-1 min-w-0">
